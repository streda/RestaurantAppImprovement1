...
app.post("/api/login", async (req, res) => {
    ...
    try {
        ...
        if (!user || user.password !== password) { // Ensure password comparison; consider hashing in production
...
app.post('/add-to-cart', authenticateToken, async (req, res) => {
    ...
    try {
      ...
      let order = await Order.findOne({ userId: req.myUser.userId, status: 'pending' });
  
      // If no pending order exists, a new order is created for the user
      if (!order) {
        order = new Order({ userId: req.myUser.userId, items: [], status: 'pending' });
      }
      ...
      */
      ...
      if (existingItemIndex > -1) { // If this item and its index is found
  ...
  */
  app.get('/cart', authenticateToken, async (req, res) => {
    try {
      const order = await Order.findOne({ userId: req.myUser.userId, status: 'pending' }).populate('items.menuItem');
...
app.post('/api/item/update', authenticateToken, async (req, res) => {
    ...
    try {
      const order = await Order.findOne({ userId: req.myUser.userId, status: 'pending' });
