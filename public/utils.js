...
//^ It just prepares the menu items from the server and store them in the menuArray for it will be used to display when "renderMenu()" function is called 
export async function fetchMenuItems(justLoggedIn = false) {
  ...
  try {
    ...
    if (justLoggedIn) {
      ...
      // If user is just switching between pages, refresh the menu items.
...
export function renderMenu(menuItems, isUserLoggedIn) {
  ...
  menuItems.forEach((menuArray_item) => {
    ...
    //* This logic is needed because it needs to render an up to date UI information upon login. It renders every item from menuArray and uses the orderArray to get the information of how many of those items have been added previously by the customer.
    const orderItem = orderArray.find((order) => 
      // Check if each of the items inside menuArray 
  ...
  if (isUserLoggedIn) {
    document.querySelectorAll(".add-btn").forEach((button) => {
      button.addEventListener("click", async (event) => {
        // The "data-item-id" is a custom HTML attribute that uses the data-* convention. The data-* attributes are used for storing extra information on HTML elements especially when that data comes from your backend. In this case it retrieves the ```data-item-id="${menuArray_item._id}``` that was assigned to each of the "add-btn" buttons of each items in the menuArray. The getAttribute() Gets the value of that attribute when the button is clicked. It returns the "id" of menu item object returned from the database
